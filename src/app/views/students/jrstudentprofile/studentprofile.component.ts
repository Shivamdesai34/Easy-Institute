...
import {EducationdetailformComponent} from "./junioreducationdetailform.component/educationdetailform.component";
...
export class StudentprofileComponent implements OnInit {
  ...
  */
  ...
  getdatafromchildEducation(e: any) {
    this.edu_submitted = e
    this.getPersonalInfo()
  }

  xlsxUpload_dyidA(element: any) {
    this.dyid_A = element.target.files[0];
  }

  xlsxUpload_dyidB(element: any) {
    this.dyid_B = element.target.files[0];
  }
  ...
  onSavedetails() {
    // if (this.oSession.registeradmissionboard == 'JR') {
    //     console.log('mm', this.studentDetailsForm.controls['dydeno'].value)
    //     if (this.studentDetailsForm.controls['dydeno'].value == null ||
    //         this.studentDetailsForm.controls['dydeno'].value == '') {
    //         this.globalmessage.Show_error('Please enter DYDE Application number')
    //         return
    //     }
    //     if (this.dyid_A == null) {
    //         this.globalmessage.Show_error('Please upload DYDE-A')
    //         return
    //     }
    //     if (this.dyid_B == null) {
    //         this.globalmessage.Show_error('Please upload DYDE-B')
    //         return
    //     }
    // }
    ...
    formdata.append(DYDEA, this.dyid_A);
    formdata.append(DYDEB, this.dyid_B);
    this.commonService
  ...
  IU_Parents() {
    ...
    this.commonService
      .Post_json_data<boolean>(Students_url.IU_Parents, input_json)
      .subscribe((response) => {
        if (response == null) {
          this.globalmessage.Show_error('Please check your details');
          return
        }
        this.IU_parentdetails = response.data;
        if (this.IU_parentdetails) {
          this.getPersonalInfo();
  ...
  IU_Address() {
    ...
    this.commonService
      .Post_json_data<boolean>(Students_url.IU_Address, input_json)
      .subscribe((response) => {
        if (response == null) {
          this.globalmessage.Show_error('Please check your details');
          return
  ...
  IU_Nationalty() {
    ...
    this.commonService
      .Post_json_data<boolean>(Students_url.IU_Nationalty, input_json)
      .subscribe((response) => {
        if (response == null) {
          this.globalmessage.Show_error('Please check your details');
          return
  ...
  IU_Others() {
    ...
    this.commonService
      .Post_json_data<boolean>(Students_url.IU_Others, input_json)
      .subscribe((response) => {
        if (response == null) {
          this.globalmessage.Show_error('Please check your details');
          return
  ...
  validateApparid(event: any) {
    const input = event.target.value;
    event.target.value = input.replace(/\D/g, ''); // remove non-digit characters
    this.studentDetailsForm.get('apaarid')?.setValue(event.target.value, {emitEvent: false});
  }
  ...
  protected readonly Global_CurrentFinYear = Global_CurrentFinYear;
}
