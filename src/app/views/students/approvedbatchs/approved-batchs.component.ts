...
import {
  IRes_batchviewsubject,
  Ires_Courseapplied,
  IRes_profileprint,
  Ires_studentapprovelist
} from "../../../models/response";
import {
    ButtonDirective, CardBodyComponent, CardComponent,
    ColComponent, IColumn,
    RowComponent,
    SmartTableComponent,
    TableColorDirective, TemplateIdDirective,
} from "@coreui/angular-pro";
import {UDownloadfiles} from "../../../globals/global_utility";
import {ApprovedbatchService} from "./approvedbatchs.service";


@Component({
  selector: 'app-approvedbatchs',
  templateUrl: './approved-batchs.component.html',
  styleUrls: ['./approved-batchs.component.scss'],
    imports: [
        ReactiveFormsModule,
        RowComponent,
        ColComponent,
...
export class ApprovedBatchsComponent implements OnInit {
  ...
  batchsubject_view_list: IRes_batchviewsubject[] = []

  columns: (IColumn | string)[] = [
    {key: 'Levelnomenclature', filter: false, sorter: false, _classes: 'text-muted small'},

    {key: 'Otherlevel', filter: false, sorter: false, _classes: 'text-muted small'},
  ];
  ...
  get_approvedbatchs() {
    ...
    this.commonService.Post_json_data<Ires_studentapprovelist[]>(Students_url.student_approvedlist, jsonin).subscribe(response => {
      if (response == null) {
  ...
  ProfilePrint() {
    ...
    this.approvedbatchservice
      .PrintProfile(jsonin_input)
      .subscribe((response) => {
  ...
  batchsubject_view(){
    ...
    this.commonService
        .Post_json_data<IRes_batchviewsubject[]>(Students_url.batchsubject_view,jsonin)
        .subscribe((response) => {
          this.batchsubject_view_list = response.data
        })
  }
}
